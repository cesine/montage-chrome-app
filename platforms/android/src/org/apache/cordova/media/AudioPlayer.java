...
public class AudioPlayer implements OnCompletionListener, OnPreparedListener,
	...
	public AudioPlayer(AudioHandler handler, String id, String file) {
		Log.v("DebuggingAudio", "New AudioPlayer " + file);
		...
		if (Environment.getExternalStorageState().equals(
			...
			this.tempFile = Environment.getExternalStorageDirectory()
					.getAbsolutePath() + "/tmprecording.3gp";
		} else {
			this.tempFile = "/data/data/"
					+ handler.cordova.getActivity().getPackageName()
	...
	public void startRecording(String file) {
		...
		case PLAY:
			...
			this.handler.webView
					.sendJavascript("cordova.require('org.apache.cordova.media.Media').onStatus('"
							+ this.id
							+ "', "
							+ MEDIA_ERROR
							+ ", { \"code\":"
	...
	private boolean readyPlayer(String file) {
		if (playMode()) {
			...
			case MEDIA_LOADING:
				...
				Log.d(LOG_TAG,
						"AudioPlayer Loading: startPlaying() called during media preparation: "
	 ...
	 */
	private void loadAudioFile(String file) throws IllegalArgumentException,
		...
		if (this.isStreaming(file)) {
			...
			try {
				...
				file = "www/"+file;
				try{
					android.content.res.AssetFileDescriptor fd = this.handler.cordova.getActivity().getAssets().openFd(file);
					...
					Log.v("DebuggingAudio",
							"Exception while preparing the audio file and assuming its in the assets folder " + file);
					ex.printStackTrace();
